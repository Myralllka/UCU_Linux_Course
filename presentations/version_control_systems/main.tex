...
\framesplitc{Branches}{graphics/branch.jpg}{
    \begin{itemize}
        \item\ex{Branch}is a divergence from the one development line to fulfill some specific purpose
...
\begin{frame}{Issues}
    \begin{itemize}
        ...
        \item \ex{Issues} are part of the development workflow, so it's important to keep them as much as keep source code of the program itself, and it's commit history
    \end{itemize}
\end{frame}
...
\framesplitc{Fork}{graphics/fork.png}{
    \begin{itemize}
        ...
        \item Forked repo can be\ex{detuched}(in theory, but it is not possible for now. By hands, the support team can do that) to became an independent project
    \end{itemize}
}

\framesplitc{Pull request}{graphics/pr.png}{
    \begin{itemize}
        \item In most cases, you don't want strangers to break all your code, so only a few people (\ex{contributors}) have right for changing the repo
...
\framesplitc{GitHub Actions. CI from GitHub}{graphics/ci.png}{
    \begin{itemize}
        ...
        \item Using this tool, it is easy to test your code for compilation on different compilers, distros, with different configurations, and all of that - automatically 
        \item There are hundreds of existing\ex{actions}, but it's possible (and quite easy) to make your own
        \item I totally recommend to use it for lab works and small projects - than it will be easier to use it everywhere
    \end{itemize}
}
