...
\begin{frame}{Overview of the most important filesystems}
    ...
    \begin{itemize}
        ...
        \item \ex{FAT} - File Allocation Table fs. There are FAT16, 32, 64. Nowedays wide used on the USB flash drives. More info and work with this fs on \ex{Operating systems} course
        \item \ex{NTFS} - fs used only on \ex{windows} os, so it is a proprietary journaling file system, But it can be mount to Linux OS (so you can access your win files from linux in case of dualboot)
    \end{itemize}
\end{frame}
...
\begin{frame}
    \begin{itemize}
        ...
        \item \ex{/boot} - boot loader files are here. 
    \end{itemize}
\end{frame}
